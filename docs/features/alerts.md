...
> ℹ️ NOTE: The other 4 locales (`it`, `id`, `zh-CN`, `zh-TW`) do not yet have the `alerts.*` i18n block that EN has in `public/locales/en/monitoring.json` — non-EN users see English strings on this page (i18n `fallbackLng: "en"` covers it functionally). Their old `email_alerts`/`sms_alerts` blocks are now dead keys. This includes the reboot-alert keys added in this feature (`event_reboot_*`, `reboot_history_*`, `reboot_cause_*`) — EN-only, same as the rest of the page. A parity sweep is a follow-up; per project convention, backfill `zh-TW` from `zh-CN` via OpenCC rather than machine-translating from EN.
