...
- **Alerts can now tell you when your modem reboots, and why.** A new opt-in reboot alert (off by default — turn it on per channel from the Alerts routing matrix) is sent once your modem is back online after any restart, labeled as a watchdog recovery, a planned reboot, or unexpected. The Alerts page also gets a new Recent Reboots list showing your device's recent restarts and their causes at a glance, whether or not you've turned the alert on.
