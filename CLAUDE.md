...
| SMS Forwarding (`qmanager_sms_forward` daemon, `cellular/sms_forwarding.sh`), Call Forwarding (`cellular/call_forwarding.sh`, `AT+CCFC`), seed-on-first-run invariant, loop guard, 3-attempt abandon, `cf_network_rejected` CME-257 first-class state, `quecmanager.sms_forwarding.*`, `quecmanager.call_forwarding.last_number` | [`docs/features/sms-call-forwarding.md`](docs/features/sms-call-forwarding.md) |
