...
| Connection Quality settings, `qmanager_ping` HTTP-probe daemon, ping profiles (sensitive/regular/relaxed/quiet), `system/ping_profile.sh` CGI (GET adds `interval_override` + `effective_interval`; POST does NOT write `interval_override`), `system/quality_thresholds.sh` CGI (POST touches both `qmanager_quality_reload` AND `qmanager_watchcat_reload` — dual reload-flag invariant), `quecmanager.ping_profile.*` (incl. `interval_override` owned solely by watchdog), `quecmanager.quality_thresholds.*` (shared with watchdog; `custom` preset + `latency_custom_ms`/`loss_custom_pct`), `avg_latency_ms` windowed-average basis for latency events (unified from `latency_ms`), `isDefault` absence semantics | [`docs/features/connection-quality.md`](docs/features/connection-quality.md) |
...
| Connection Watchdog, `qmanager_watchcat`, watchdog daemon, `monitoring/watchdog.sh`, `use-watchdog-settings.ts`, dual-trigger model (reachability + quality), `fail_threshold` (renamed from `max_failures`; counts raw `ping_streak_fail` probes), `interval_override` single-writer rule (watchdog is sole writer of `ping_profile.interval_override`), `effective_interval` resolution (override ?? profile→seconds map), `probe_profile`, shared `quality_thresholds.*` (retired `latency_ceiling_ms`/`loss_ceiling_pct`), `avg_latency_ms` windowed-average basis, `quality_enabled`, `quality_consecutive`, `STATUS_STALE_THRESHOLD`, float-comparison awk rule, reason-aware cooldown (`finish_cooldown` top guard for `primary_recheck_active` arm → `revert_settle_active` arm → reason-aware arm), dual reload-flag invariant (`quality_thresholds.sh` touches both `qmanager_quality_reload` + `qmanager_watchcat_reload`), Tier 1 AT+COPS re-registration, Tier 1→4 recovery ladder, recovery-lifecycle Network Events (reuse existing `watchcat_recovery`/`sim_failover` types), auto-disable token bucket, SSR-aware hold (`ssr_aware`, `ssr_grace`, `ssr_hold`, `ssr_in_progress`, `ssr_hold_gate`, `last_ssr_detected`), MPSS SSR amplification, dmesg-evidence mechanism, grace-window fall-through, `WatchcatState` `ssr_hold`, SIM-failover lifecycle (`SIM_SETTLE_SECS=90` hard-coded settle floor for Tier-3 + revert + recheck — not a UCI key; `finalize_sim_failover` shared finalizer called from BOTH reachability and quality success arms; `/tmp/qmanager_sim_failover` `.reason` field = `"connectivity_failure"` or `"quality_degradation"`; `original_iccid` pre-swap ICCID best-effort 3×retry; `revert_settle_active` post-revert hold variable; `primary_recheck_enabled`/`primary_recheck_interval` UCI keys for auto-failback; `primary_recheck_active` in-process shell variable (not in state file); `initiate_primary_recheck` blind slot-swap + settle; `watchdog_failback` auto_apply_profile reason; `check_revert_request` 3-branch pending/defer/delete logic; `sim_db_add` on all swap/revert/fallback/recheck paths incl. slow-modem path; 3×1s ICCID retry; retired `last_iccid` → `sim_db_add`; `--argjson`-vs-`--arg` jq boolean gotcha — see Known Gotchas) | [`docs/features/connection-watchdog.md`](docs/features/connection-watchdog.md) |
...
| Adaptive Polling, `qmanager_poller` AT backoff, `system/adaptive_polling.sh`, `quecmanager.poller.*` (never seeded), `/tmp/qmanager_ui_active` heartbeat, `/tmp/qmanager_poller_reload`, `/tmp/qmanager_force_tier2`, `.device.poller_tier` ∈ `active`\|`idle`\|`deep`, tier cadence (active 2s / idle 15s / deep 60s defaults), AT-due gating (`cycle_count % (interval/POLL_INTERVAL)`), full-refresh on idle touch, `write_cache` always-run invariant (watchdog dependency), force-tier2 apply chokepoints (`qmanager_profile_apply`; `apn_mgr.sh` now uses `ondemand_dataplane_refresh` instead), boot-seed invariant; Tier-2 is now identity-only (data-plane/L1 group fully removed — see on-demand radio details); deep tier runs QENG only (`QCAINFO`/`CFUN?`/per-antenna gated to active+idle; CA fields retain last-known in deep — do NOT move the QCAINFO else-branch outside the `!= "deep"` guard); QENG kept in deep solely to feed `qmanager_tower_failover` `.lte.rsrp`/`.nr.rsrp` | [`docs/features/adaptive-polling.md`](docs/features/adaptive-polling.md) |
| IP Passthrough (IPPT), `network/ip_passthrough.sh`, `/etc/qmanager/ippt_config.json`, `parse_ippt_nat`/`parse_ippt_usbnet`/`parse_ippt_dhcpv4dns`/`parse_ippt_mpdn_rule` in `parse_at.sh`, `qcmd` trailing-comma anchor invariant, Verizon profile lock (`ip_passthrough_locked_by_verizon_profile`), apply-then-reboot one-shot (no separate reboot action), MPDN_rule field layout, boot cache `.device.ippt_*` | [`docs/features/ip-passthrough.md`](docs/features/ip-passthrough.md) |
| On-Demand Radio Details, `cellular/radio_details.sh` (GET), `ondemand_radio.sh` lib (`load_ondemand_cache`, `ondemand_radio_fetch`, `ondemand_dataplane_refresh`), `/tmp/qmanager_ondemand.json` cache, RAT-gate hardening (serving-cell re-read gates mimo; skip on unknown/transitioning RAT), apply-chokepoint data-plane refresh (`apn_mgr.sh` + `qmanager_profile_apply`), `useRadioDetails` 7 s mount-scoped poll, `stale:true` last-known fallback, `RadioDetails`/`RadioDetailsResponse` types | [`docs/features/ondemand-radio-details.md`](docs/features/ondemand-radio-details.md) |
