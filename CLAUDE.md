...
| SMS inbox/send/delete (`cellular/sms.sh`), SMS alerts (`sms_alerts.sh`), patched `sms_tool`, `/dev/smd11` invariant, `AT+CPMS` storage routing, dual ME+SM merge, `qmanager_sms_storage` boot daemon | [`docs/features/sms.md`](docs/features/sms.md) |
...
| Connection Quality settings, `qmanager_ping` daemon, ping profiles, `system/ping_profile.sh` + `system/quality_thresholds.sh` CGI, `quecmanager.ping_profile.*` / `quecmanager.quality_thresholds.*`, `avg_latency_ms` | [`docs/features/connection-quality.md`](docs/features/connection-quality.md) |
